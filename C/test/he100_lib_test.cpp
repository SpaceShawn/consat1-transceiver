...
// store valid response, should return 1
TEST_F(Helium_100_Test, StoreValidResponse2)
{
...
// store valid response, should return 1
TEST_F(Helium_100_Test, StoreValidResponse3)
{
...
TEST_F(Helium_100_Test, GetConfig)
{
    unsigned char sample_config[44] = {0x48,0x65,0x20,0x05,0x00,0x22,0x47,0xb1,0x00,0x87,0x01,0x01,0x00,0x00,0xa8,0x3c,0x02,0x00,0x08,0xab,0x06,0x00,0x43,0x51,0x20,0x20,0x20,0x20,0x4e,0x4f,0x43,0x41,0x4c,0x4c,0x05,0x00,0x00,0x00,0x41,0x80,0x00,0x00,0xfa,0x10};
    struct he100_settings test_settings = HE100_parseSettings(sample_config);
    ASSERT_EQ (0x00, test_settings.led_blink_type);   
TEST_F(Helium_100_Test, TestPrepareConfig)
{
    unsigned char good_config[44] = {0x48,0x65,0x10,0x06,0x00,0x22,0x38,0x74,0x00,0x00,0x01,0x01,0x00,0x00,0x48,0x33,0x02,0x00,0x98,0x93,0x06,0x00,0x56,0x41,0x33,0x4F,0x52,0x42,0x56,0x45,0x32,0x43,0x55,0x41,0x09,0x00,0x00,0x00,0x43,0x00,0x00,0x00,0x33,0x78}; 
    struct he100_settings good_settings = HE100_prepareConfig(good_config);

    ASSERT_EQ(9600, good_settings.rx_rf_baud_rate); // TODO test all like this to known good config
}

TEST_F(Helium_100_Test, TestValidateConfig)
{
    unsigned char good_config[44] = {0x48,0x65,0x10,0x06,0x00,0x22,0x38,0x74,0x00,0x00,0x01,0x01,0x00,0x00,0x48,0x33,0x02,0x00,0x98,0x93,0x06,0x00,0x56,0x41,0x33,0x4F,0x52,0x42,0x56,0x45,0x32,0x43,0x55,0x41,0x09,0x00,0x00,0x00,0x43,0x00,0x00,0x00,0x33,0x78}; 
    struct he100_settings good_settings = HE100_prepareConfig(good_config);

    unsigned char * validated_config = HE100_validateConfig(good_settings);

    ASSERT_NE(-1, validated_config[0]); 
}
